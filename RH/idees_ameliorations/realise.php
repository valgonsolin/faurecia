...
if(isset($_GET['vote'])){
  ...
  if($a_vote>0){
      $Qy = $bdd->prepare('SELECT *  FROM idees_ameliorations WHERE id= ? and date_val IS NOT NULL ');
      $Qy->execute(array($a_vote));
      if($Qy->fetch()){warning("ERREUR","vous avez deja classé cette idée comme 'réalisée' ");
      }else{$queryy=$bdd->prepare('UPDATE idees_ameliorations SET date_val= ? WHERE id =? ');
            $queryy -> execute(array( $datetime,$a_vote));
            success("SUCCES","L'idée est maintenant considérée comme réalisée ") ;}
  }else{$Qy = $bdd->prepare('SELECT *  FROM idees_ameliorations WHERE id= ? and date_val IS NULL ');
  $Qy->execute(array($a_vote));
  if($Qy->fetch()){warning("ERREUR","L'idée est déja considérée comme 'non réalisée'  ");
  }else{$p=(-$a_vote);
         $qy=$bdd->prepare('UPDATE idees_ameliorations SET date_val= ? WHERE id = ?');
         $qy -> execute(array($datetime,$p));
...
<h2>Idée n°<?php echo $idee ; ?> </h2>
  ...
  <?php
    ...
    <div class="row"> <div class="col-md-2"> <a href="realise.php?vote=<?php echo ($idee); ?>" class="btn btn-default pull-right">Classer l'idée comme 'realisee' </a> </div>
...
<h2>Idée n°<?php echo $idee ; ?> </h2>
  ...
  <?php
    ...
    <div class="row"> <div class="col-md-2"> <a href="realise.php?vote=<?php echo (-$idee); ?>" class="btn btn-default pull-right">Classer l'idée comme non 'realisee'</a> </div>
    <div class="col-md-5"></div>
    <div class="col-md-5">
      <?php
        if($emm['image2'] != NULL){
