...
<?php
  ...
  <a href="ajout.php" class="btn btn-default pull-right">Ajouter une formation</a>
...
$Query = $bdd->prepare('SELECT * FROM formations_dispo JOIN demande_formations ON formations_dispo.id=demande_formations.formation
  JOIN evaluation_froid ON demande_formations.id=evaluation_froid.demande WHERE trainingtitle LIKE :tt AND DATEDIFF(:d1,date_fin )>0  AND valide=1 AND evalfroid=1 ORDER BY date_deb LIMIT 20  OFFSET :nb') ;
  ...
  <div class="alerte" >
      ...
      <div class="info_alerte">
          ...
          <p><b>Demandeur : </b><?php echo $DA['nom']." ".$DA['prenom'] ; ?> <br>
              ...
              <b>Evaluation du manager : Objectifs </b><?php echo $Data['resultats'] ;?>
...
$test = $bdd->prepare('SELECT * FROM formations_dispo JOIN demande_formations ON formations_dispo.id=demande_formations.formation WHERE trainingtitle LIKE :tt AND DATEDIFF(date_deb, :d1 )>0  AND valide=1 AND evalfroid=1 ORDER BY date_deb LIMIT 20  OFFSET :nb');
