...
else
{
  ...
  $lastOrdre=-1;
  $query= $bdd -> query('SELECT * FROM formation_question ORDER BY ordre DESC LIMIT 1');
  ...
  if(!empty($_POST)){
    ...
    if($_FILES['file_1']['name'] != ""){
      $id1=upload($bdd,'file_1',"../../../ressources","hse",5048576,array( 'jpg' , 'jpeg' , 'gif' , 'png' , 'JPG' , 'JPEG' , 'GIF' , 'PNG' ));
      if($id1>=0){
        $reponse1="img=".$id1;
      }
    }
    if($_FILES['file_2']['name'] != ""){
      $id2=upload($bdd,'file_2',"../../../ressources","hse",5048576,array( 'jpg' , 'jpeg' , 'gif' , 'png' , 'JPG' , 'JPEG' , 'GIF' , 'PNG' ));
      if($id2>=0){
        $reponse2="img=".$id2;
      }    }
    if($_FILES['file_3']['name'] != ""){
      $id3=upload($bdd,'file_3',"../../../ressources","hse",5048576,array( 'jpg' , 'jpeg' , 'gif' , 'png' , 'JPG' , 'JPEG' , 'GIF' , 'PNG' ));
      if($id3>=0){
        $reponse3="img=".$id3;
      }    }
    if($_FILES['file_4']['name'] != ""){
      $id4=upload($bdd,'file_4',"../../../ressources","hse",5048576,array( 'jpg' , 'jpeg' , 'gif' , 'png' , 'JPG' , 'JPEG' , 'GIF' , 'PNG' ));
      if($id4>=0){
        $reponse4="img=".$id4;
      }    }
    $query = $bdd -> prepare('INSERT INTO formation_question(type,titre,question,reponse_1,reponse_2,reponse_3,reponse_4,corrige_1,corrige_2,corrige_3,corrige_4,ordre,image_correction,commentaire) VALUES (:type,:titre,:question,:reponse_1,:reponse_2,:reponse_3,:reponse_4,:corrige_1,:corrige_2,:corrige_3,:corrige_4,:ordre,:file,:commentaire)');
    $id= $bdd -> lastInsertId();
    $query -> execute(array(
      'type' => $_POST['type'],
      'titre' => $_POST['titre'],
