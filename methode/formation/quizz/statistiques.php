...
<?php
$Qy2 = $bdd->query('SELECT DISTINCT personne FROM formation_session WHERE valide=1 ');
$nombre_valide=0;
while($n2 = $Qy2->fetch()){
  $nombre_valide+=1; }
?>
...
$Query = $bdd->query('SELECT id,titre,question FROM formation_question ');
...
while ($Data = $Query->fetch()) {
  ...
  WHERE profil.supprime = 0 AND formation_reponse.question = ? ');
