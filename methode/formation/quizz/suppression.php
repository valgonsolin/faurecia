...
else
{
  ...
  if(isset($_POST['reset'])){
    $query= $bdd -> query('SELECT * FROM formation_question');
    $i=0;
    while($Data = $query -> fetch()){
      $update=$bdd -> prepare('UPDATE formation_question SET ordre=? WHERE id=?');
  ...
  if(isset($_POST['supprimer'])){
    ...
    $query = $bdd -> prepare('DELETE FROM formation_question WHERE id=?');
    $query -> execute(array($_POST['id']));
    $query = $bdd -> prepare('UPDATE formation_question SET ordre=ordre-1 WHERE ordre > ?');
  ...
  $Data= $query -> fetch();
  remove_file($bdd,$Data['image_correction']);
  $query = $bdd -> query('UPDATE formation_question SET image_correction = NULL WHERE id = ?');
  $query -> execute(array($_POST['id']));
  ...
  $query = $bdd -> prepare('UPDATE formation_question SET type = :type,titre= :titre,question = :question,reponse_1 = :reponse_1,reponse_2 = :reponse_2,reponse_3 = :reponse_3,reponse_4 = :reponse_4,corrige_1 = :corrige_1,corrige_2 = :corrige_2,corrige_3 = :corrige_3,corrige_4 = :corrige_4, ordre= :ordre, image_correction= :image_correction, commentaire= :commentaire WHERE id = :id');
  $query -> execute(array(
    'type' => $_POST['type'],
    'titre' => $_POST['titre'],
  ...
  if($_POST['ordre1'] > $_POST['ordre2']){
    $query = $bdd -> prepare('UPDATE formation_question SET ordre=ordre+1 WHERE ordre >= ? AND ordre < ? AND id <> ?');
...
$nb=$_GET['nb'];
  ...
  $query=$bdd -> prepare('SELECT * FROM formation_question WHERE (question LIKE :question or titre LIKE :titre) ORDER BY ordre LIMIT 20 OFFSET :nb');
...
$test = $bdd->prepare('SELECT * FROM formation_question WHERE (question LIKE :question or titre LIKE :titre) LIMIT 1 OFFSET :nb');
