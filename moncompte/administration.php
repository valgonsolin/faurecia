...
if (isset($_POST['modifier'])){
    if (isset($_POST['id'])){
        $Query = $bdd->prepare('UPDATE profil SET nom = ?, prenom = ?,identifiant = ?, mo = ?, uap = ?, tournee = ?, admin = ?, qualite = ?, rr = ?, hse = ?, kamishibai = ? WHERE id = ?');
        if($Query->execute(array($_POST['nom'],$_POST['prenom'],$_POST['identifiant'],$_POST['mo'],$_POST['uap'],$_POST['tournee'],$_POST['admin'],$_POST['qualite'],$_POST['rr'],$_POST['hse'],$_POST['kamishibai'],$_POST['id']))){
        ...
        if($_POST['new1'] == $_POST['new2']){
          $Query = $bdd->prepare('INSERT INTO profil SET nom = ?, prenom = ?,identifiant = ?, mo = ?, uap = ?, tournee = ?,password = ?');
          if($Query->execute(array($_POST['nom'],$_POST['prenom'],$_POST['identifiant'],$_POST['mo'],$_POST['uap'],$_POST['tournee'], crypt($_POST['new1'],"faureciabeaulieu")))){
  ...
  if(!$_SESSION['admin']){
    ...
    <form class="form-inline">
      <div class="form-group">
        ...
        <label style="margin: 30px;"><input type="checkbox" name="supprime" <?php if($supprime){echo "checked";} ?>>  Supprimé</label>
      </div>
